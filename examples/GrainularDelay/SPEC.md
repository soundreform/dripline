...
1. **Read:** Sum all active true-stereo grains into a master `grain_mix_L` and `grain_mix_R` (applying a 0.25f gain compensation).
...
- **Block-Rate Updates:** Execute ADC reads and smoothing math exactly once per audio callback block.
...
2. **Duration:** Duration of the grain, or "grain size" (10ms to 500ms).
...
4. **Modulation Depth:** Some / None / More (Controls an internal LFO that swells the chaos and density intensities)
...
- **Visual Confirmation:** Upon a successful mode toggle at boot, **LED 1** should flash rapidly for 200ms before entering the main application loop.
- **Storage Mechanism:** Use `daisy::PersistentStorage` to manage the settings block in Flash memory.

**Density Knob & Scheduler Logic:**

The behavior of **Knob 3 (Density)** is determined by the **Density Rhythm** toggle switch and whether a tap tempo is active.

- **When Rhythm is `Free (Async)`:**
  - The `Density` knob directly controls the trigger frequency, from sparse clicks to dense clouds. Tap tempo has no effect in this mode.

- **When Rhythm is `Synced` or `Burst`:**
  - The `Density` knob's function changes to a **subdivision selector** for the tempo provided by **Footswitch 2 (Tap Tempo)**. The base frequency is set by tapping, and the knob selects how to divide that beat.
...
### **10. Expression Pedal Integration**

An analog expression pedal can be routed to select core parameters to allow for hands-free manipulation.

- **Assignment Gesture:**
  This gesture is only available when the effect is **Active** (LED 1 is Green). Hold **FOOTSWITCH_2** (Tap/Clear) for >1 second. While held, **KNOB_10** selects the target parameter (quantized into distinct zones). The available targets are: Unassigned (White), Pitch (Blue), Duration (Green), Density (Yellow), Time Spray (Cyan), Pitch Spray (Magenta), Feedback (Orange). **LED_2** changes to the target's respective color on change. Assignment happens when **FOOTSWITCH_2** is released. **LED_2** will blink the target color to confirm the change.
- **Sweep Control (KNOB_10):**
  When **FOOTSWITCH_2** is not held, Knob 10 dictates the depth and polarity of the expression pedal sweep relative to the assigned knob's base position.
  - **Center (0.5):** Expression pedal has no effect.
  - **Right (0.5 to 1.0):** Toe-down sweeps the parameter up to a maximum defined by the depth.
  - **Left (0.0 to 0.5):** Toe-down sweeps the parameter _downward_ (inverted sweep).
- **ADC Processing:** The expression input must be processed through the same Block-Rate One-Pole Lowpass Filter as the potentiometers to prevent zipper noise during rapid foot movements.
