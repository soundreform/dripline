...
- **Assignment Gesture:**
  Hold **FOOTSWITCH_2** (Tap/Clear) for >1 second. While held, **KNOB_10** selects the target parameter (quantized into distinct zones). The available targets are: Unassigned (White), Pitch (Blue), Grain Size (Green), Density (Yellow), Time Spray (Cyan), Pitch Spray (Magenta), Feedback (Orange). **LED 1** flashes the target's respective color rapidly to confirm the assignment change.
